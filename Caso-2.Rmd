...
paga <- lending_base %>% filter(estado_pago == "Pagado") %>% sample_n(5000)
nopaga <- lending_base %>% filter(estado_pago == "Incumplido") %>% sample_n(5000)
...
Base_datos= bind_rows(paga, nopaga) %>% 
  ...
  rename(Ingreso=2,"Relacion_deuda_ingreso"=3,"Monto_prestado"=4,
         FICO=5,Proposito=6,Estado=7)
...
Base_datos_formateada <- Base_datos %>%
  mutate(
    Ingreso = scales::dollar(Ingreso, accuracy = 1),
    Monto_prestado = scales::dollar(Monto_prestado, accuracy = 1),
    Relacion_deuda_ingreso = scales::percent(Relacion_deuda_ingreso/100, accuracy = 0.1),
...
tabla_interactiva <- Base_datos %>%
  ...
  formatCurrency(
    columns = c('Ingreso', 'Monto prestado'),
    currency = '$', digits = 0, before = TRUE
  ) %>%
  formatPercentage(
    columns = 'Relacion deuda/ingreso',  # Usar nombre SIN espacios
    digits = 1
  ) %>%
  formatRound(
    columns = 'FICO',
