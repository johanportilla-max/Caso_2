...
# Función corregida para crear boxplots estáticos
crear_boxplot_estatico <- function(var_nombre, titulo_display) {
  ...
  p <- plot_ly() %>%
    ...
    layout(
      title = list(
        text = titulo_display,  # TÍTULO INDIVIDUAL PARA CADA GRÁFICA
        font = list(
          family = "Playfair Display",
          size = 16,
          color = SC["primary_red"]
      ...
      plot_bgcolor = SC["parchment"],
      paper_bgcolor = "white",
      margin = list(l = 70, r = 30, t = 80, b = 60),  # Aumenté top margin para el título
      showlegend = FALSE,
...
# Crear cada subplot individualmente con títulos específicos
...
p_comparativo_final <- subplot(
  ...
  shareX = TRUE,
  titleY = TRUE,
  margin = 0.08  # Aumenté el margen para acomodar los títulos
) %>%
  layout(
    title = list(
      ...
      x = 0.5,
      y = 0.98  # Posicionar el título principal más arriba
    ),
    font = list(family = "Source Serif Pro", size = 12),
    annotations = list(
      # Eje X compartido
  ...
  config(
    displayModeBar = FALSE,  # DESACTIVAR LA BARRA DE HERRAMIENTAS (estático)
    staticPlot = TRUE        # HACER EL PLOT COMPLETAMENTE ESTÁTICO
  )
