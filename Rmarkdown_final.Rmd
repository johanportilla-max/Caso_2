...
```{r resultados_tbl, echo=FALSE, message=FALSE, warning=FALSE} 
library(knitr) 
library(kableExtra) 
# Definimos la tabla con las variables y descripciones 
variables <- data.frame(
  Variable = c("estado_pago", "ingreso", "relacion_deuda_ingreso", "monto_prestamo", "puntaje_fico", "proposito_agrupado", "default_num"),
  Descripción = c(
  ...
  Ejemplos / Notas = c( 
    "Ej: 'pagado', 'impago'",
    "En dólares anuales", 
    "Proporción, por ejemplo 0.3", 
    "En dólares", 
    "Rango típico 300-850",
    "Ej: 'vivienda', 'deuda educación', 'personal'",
    "0 = pagado, 1 = impago" ), check.names = FALSE, stringsAsFactors = FALSE ) 
# Crear la tabla con kableExtra replicando estilo del informe 
variables %>% kable("html", escape = FALSE, align = c("l","l","c","l"), col.names = c("Variable", "Descripción", "Tipo de Variable", "Ejemplos / Notas")) %>% kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover", "condensed", "responsive"), font_size = 14) %>% row_spec(0, background = "#4682B4", color = "white", bold = TRUE) %>% column_spec(1, bold = TRUE) %>% scroll_box(height = "350px") $$ P(\text{default\_num} = 1 \mid \text{ingreso}, \text{relacion\_deuda\_ingreso}, \text{monto\_prestamo}, \text{puntaje\_fico}, \text{proposito\_agrupado}) = \frac{1}{1 + e^{- \left( \beta_0 + \beta_1 \cdot \text{Ingreso} + \beta_2 \cdot \text{Relación Deuda-Ingreso} + \beta_3 \cdot \text{Monto Préstamo} + \beta_4 \cdot \text{Puntaje FICO} + \beta_5 \cdot \text{Propósito Agrupado} \right)}} $$
```
...
```
>>>>>>> 1e9fc8959be2e2a58e046fff433bee4ebe68fbee
